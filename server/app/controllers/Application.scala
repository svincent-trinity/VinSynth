...
class Application @Inject() (protected val dbConfigProvider: DatabaseConfigProvider, cc: MessagesControllerComponents)(implicit ec: ExecutionContext)
  ...
  def keyboard = Action { implicit request =>
    Ok(views.html.keyboard(SharedMessages.itWorks))
  }
  ...
  def withJsonBody[A](f: A => Future[Result])(implicit request: Request[AnyContent], reads: Reads[A]): Future[Result] = {
    request.body.asJson.map {body => 
          Json.fromJson[A](body) match {
              ...
              case e @ JsError(_) => Future.successful(Redirect(routes.Application.keyboard()))
          }
       }.getOrElse(Future.successful(Redirect(routes.Application.keyboard())))
